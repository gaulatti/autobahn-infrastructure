...
const extractLighthouseSummary = (rawData: string, mode: 'desktop' | 'mobile') => {
  ...
  const simplifiedResult: SimplifiedLHResult = {
    ...
    performance: lhReport.categories.performance?.score * 100 || 0,
    accessibility: lhReport.categories.accessibility?.score * 100 || 0,
    bestPractices: lhReport.categories['best-practices']?.score * 100 || 0,
    seo: lhReport.categories.seo?.score * 100 || 0,
    timings: {
